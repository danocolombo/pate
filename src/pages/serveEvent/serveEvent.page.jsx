...
import NumberUpdateModal from "../../components/modals/serve-event/serve-event-number-update.modal";
...
const Serve = ({
  ...
  const [isEventNumberModalVisible, setIsEventNumberModalVisible] =
    useState(false);
    ...
    if (match?.params?.id) {
      ...
      if (match.params.id === "0") {
        ...
        async function getEventDetails() {
          ...
          API.graphql(graphqlOperation(queries.getEventDetails, variables))
            .then((gqlResponse) => {
            ...
            .catch((error) => {
              printObject("SEP:125--> error getting event from graphql", error);
            });
        }
        getEventDetails();
        loadRally();
  ...
  useEffect(() => {
    if (
      rallyEvent.status === "done" &&
      (parseInt(rallyEvent.actualCount) === 0 ||
        parseInt(rallyEvent.actualCount) < parseInt(rallyEvent.plannedCount) ||
  ...
  const handleSubmitClick = async (event) => {
    ...
    if (
      ...
      const constactUpdateResults = await updateGQLEventContact(contactUpdates);
    ...
    if (rallyEvent.meal === null) {
      ...
      switch (mealInsertResults.statusCode) {
        case 200:
          ...
          break;
        case 401:
          console.log("graphql completed, but unexpected results meal insert");
      ...
      switch (mealUpdateResults.statusCode) {
        case 200:
          ...
          break;
        case 401:
          console.log("graphql completed, but unexpected results meal update");
  ...
  //     history.push('/serve');
  // };
  const handleChangeGraphicClick = () => {
    //calling string
    const serveGraphicPath = `/serveGraphic/${rallyEvent.id}/${rallyEvent.graphic}`;
    history.push(serveGraphicPath);
  };
  ...
  //   printObject("SEP:877==>rallyEvent:\n", rallyEvent);
  const hasErrors =
    churchNameError !== "" ||
  ...
  return pateSystem.showSpinner ? (
    ...
    <>
          ...
          <div className="serveevent-page__data-input-box">
            {rallyEvent?.coordinator?.id !== currentUser?.id && (
              <Stack direction="row">
                ...
                >
                  Coordinator: {rallyEvent?.coordinator?.firstName}{" "}
            ...
            <Stack>
              ...
              <Select
                ...
                style={{
                  ...
                  margin: "0px",
                  fontSize: "1.2rem",
                  backgroundColor: eventStatusError ? "yellow" : null,
                }}
                value={eventStatus}
                onChange={(event) => {
                  const value = event.target.value;
                  setEventStatus(value);
                  setEventStatusEffor(validateEventStatus(value));
                }}
                sx={{
              ...
              <Stack>
                <TextField
                  ...
                  onChange={(e) => {
                    ...
                    setChurchNameError(validateChurchName(e.target.value));
                  }}
              ...
              <Stack direction="row" spacing={1}>
                <Stack>
                  <TextField
                    ...
                    select
                    value={stateProv}
                    onChange={(event) => setStateProv(event.target.value)}
                  >
                    {US_STATES.map((state) => (
                ...
                <Stack direction="row" spacing={1}>
                  <TextField
                    ...
                    onChange={(e) => {
                      ...
                      setPostalCodeError(
                        validatePostalCode(e.target.value.substring(0, 5))
                      );
                    }}
              ...
              >
                <PhoneInput
                  ...
                  onChange={(contactPhone) => {
                    ...
                    setContactPhoneError(validateContactPhone(contactPhone));
                  }}
                  inputStyle={{
                    fontSize: "20px",
                    color: "black",
              ...
              <Stack direction="row" spacing={1}>
                <TextField
                  ...
                  onChange={(e) => {
                    ...
                    setContactEmailError(validateContactEmail(e.target.value));
                  }}
            ...
            <div className="serveevent-page__section-header">Logistics</div>
            <Box
              sx={{
                border: "1px solid black",
                borderRadius: "5px",
            ...
            <div className="serveevent-page__section-header">Meal Details</div>
            <Box
              sx={{
                border: "1px solid black",
                borderRadius: "5px",
            ...
            >
              ...
              <Stack
                ...
                sx={{ marginTop: "5px" }}
              >
                <label htmlFor="meal-message">Meal Message</label>
                <TextareaAutosize
                  id="meal-message"
                  ...
                  minRows={2}
                  value={mealMessage}
                  onChange={(event) => setMealMessage(event.target.value)}
                  style={{
                    width: "90%",
                    backgroundColor: "#f5f5f5",
                    padding: "10px",
              ...
              <div className="serveevent-page__graphic-section">
                ...
                <Stack>
                  <Button
                    ...
                    onClick={handleChangeGraphicClick}
                  >
                    CHANGE IMAGE
                  </Button>
                </Stack>
            ...
            {match.params.id !== "0" ? (
              <>
                ...
                <div className="serveevent-page__registration-data-box">
                  {rallyEvent?.registrations?.items.length > 0 ? (
                    rallyEvent.registrations.items.map((rege) => (
                      <RegistrationItem key={rege.id} regItem={rege} />
                    ))
                  ) : (
      ...
      <ModalWrapper isOpened={isEventNumberModalVisible}>
        <NumberUpdateModal
          ePlanned={rallyEvent.plannedCount}
          eActual={rallyEvent.actualCount}
          mPlanned={rallyEvent.mealPlannedCount}
          mActual={rallyEvent.mealActualCount}
...
const mapDispatchToProps = (dispatch) => ({
  ...
  removeEventFromCurrentUser: (id) => dispatch(removeEventFromCurrentUser(id)),
});
// Serve.propTypes = {

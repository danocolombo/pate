...
const Serve = ({
  ...
  useEffect(() => {
    async function getDivisionEvents() {
      ...
      API.graphql(graphqlOperation(queries.getAllDivisionEvents, variables))
        .then((divisionEvents) => {
          if (divisionEvents?.data?.getDivision?.events.items.length > 0) {
            // printObject(
            //   "SP:68-->allDivisionEvents: ",
            //   divisionEvents?.data?.getDivision?.events.items
            // );
            // console.log("SET_PATE_RALLIES");
            setPateRallies(divisionEvents.data.getDivision.events.items);
          } else {
  ...
  const getRepRallies = async () => {
    ...
    API.graphql(graphqlOperation(queries.getRepRallies, variables))
      ...
      .catch((error) => {
        printObject("SP:112--> error getting rep rallies from graphql", error);
      });
  };
  const getLeadRallies = async () => {
    ...
    API.graphql(graphqlOperation(queries.getAllDivisionEvents2, variables))
      .then((allRallyResponse) => {
        if (allRallyResponse?.data?.getDivision?.events.items.length > 0) {
          ...
          let allRallies = allRallyResponse?.data?.getDivision?.events.items;
          ...
          allRallies.sort(function (a, b) {
            ...
            if (result === 0) {
              result = a.location.stateProv.localeCompare(b.location.stateProv);
            }
            return result;
          });
          ...
          allRallies.forEach((r) => {
            if (r.location.stateProv === currentUser.residence.stateProv) {
